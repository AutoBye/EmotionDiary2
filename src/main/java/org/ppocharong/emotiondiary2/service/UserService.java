...
public class UserService {
    ...
    public ResponseEntity<?> login(UserDTO userDTO, HttpSession session) {
        ...
        if (username != null && isValidPassword) {
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Collections.singletonMap("error", "로그인 실패: 이메일 또는 비밀번호가 잘못되었습니다."));
        }
    }
    ...
    public ResponseEntity<?> register(UserDTO userDTO) {
        ...
        if (userRepository.existsByUsername(username)) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(Collections.singletonMap("error", "이미 존재하는 사용자 이름입니다."));
        }

        if (userRepository.existsByEmail(email)) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(Collections.singletonMap("error", "이미 존재하는 이메일입니다."));
        }
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Collections.singletonMap("error", "회원가입에 실패했습니다."));
        }
    }
